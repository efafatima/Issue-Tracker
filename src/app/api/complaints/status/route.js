...
export async function POST(request) {
  ...
  const id = body.complaint_id || body.id;
  const newStatus = body.status;
  if (!id) return fail("Complaint id is required", 400);
  ...
  const { data, error } = await ctx.supabase
    ...
    .eq("id", id)
    .eq("assigned_teacher_id", ctx.profile.id)
    .select("*, student:user_id(id,username,email,role), department:department_id(id,name), assigned_teacher:assigned_teacher_id(id,username,email,role,faculty_designation)")
    .single();
  ...
  if (complaint.user_id) {
    await ctx.supabase.from("notifications").insert({
      ...
      complaint_id: id,
      message: `Complaint #${id} was resolved.`
    });
  }
